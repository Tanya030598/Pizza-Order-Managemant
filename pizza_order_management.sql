...
Ans-- SELECT order_id, sum(total_amount) FROM orders group by order_id;

-- Intermediate Queries --

Query 1: Get the total spent by each customer
Ans-- SELECT c.first_name, c.last_name, SUM(o.total_amount) AS total_spent
      FROM customers c
      JOIN orders o ON o.customer_id = c.customer_id
      GROUP BY c.customer_id;

Query 2: Get the details of order items for order 1
Ans-- SELECT oi.order_item_id, p.name, oi.quantity, oi.price
      FROM order_items oi
      JOIN pizzas p ON oi.pizza_id = p.pizza_id
      WHERE oi.order_id = 1;

Query 3: Get the total sales for each pizza
Ans-- SELECT p.name, SUM(oi.quantity) AS total_sales
      FROM pizzas p
...
Query 1: Get the most popular pizza (most ordered)
Ans-- SELECT p.name, SUM(oi.quantity) AS total_orders
      FROM pizzas p
      JOIN order_items oi ON oi.pizza_id = p.pizza_id
      GROUP BY p.name
...
Ans-- SELECT c.first_name, c.last_name, SUM(o.total_amount) AS total_revenue
      FROM customers c
      JOIN orders o ON o.customer_id = c.customer_id
      GROUP BY c.customer_id
...
Ans-- SELECT p.size, SUM(oi.quantity) AS total_sales
      FROM pizzas p
      JOIN order_items oi ON oi.pizza_id = p.pizza_id
      GROUP BY p.size;
