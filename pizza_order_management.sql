...
/*Get the most popular pizza (most ordered)*/
...
/*Get the most frequent order status*/
...
/*Get the total sales for each pizza size*/
SELECT p.size, SUM(oi.quantity) AS total_sales
FROM pizzas p
JOIN order_items oi ON oi.pizza_id = p.pizza_id
GROUP BY p.size;
...
/*Get all pizzas in the menu*/
SELECT * FROM pizzas;

/*Get all orders for customer 1*/
SELECT * FROM orders WHERE customer_id = 1;

/*Get all order items for order 1*/
SELECT * FROM order_items WHERE order_id = 1;

/*Get all orders and their total amounts*/
SELECT order_id, sum(total_amount) FROM orders group by order_id;
